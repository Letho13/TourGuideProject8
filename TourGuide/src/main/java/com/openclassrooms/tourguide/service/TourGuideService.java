...
//	}
    ...
    public CompletableFuture<List<NearByAttractionsByUserDTO>> getNearByAttractions(CompletableFuture<VisitedLocation> visitedLocation) {
...
//                        int reward = rewardsService.getRewardPoints(attraction, user);
                    ...
                    .sorted(Comparator.comparingDouble(NearByAttractionsByUserDTO::getDistanceBetweenUserAndAttraction))
                    .limit(5)
                    .toList();
...
//    }
    ...
    private CompletableFuture<Void> initializeInternalUsers() {
        ...
        IntStream.range(0, InternalTestHelper.getInternalUserNumber()).forEach(i -> {
            ...
            CompletableFuture<Void> fullProcess = CompletableFuture
                    .supplyAsync(() -> new User(UUID.randomUUID(), userName, phone, email))
                    .thenCompose(user -> CompletableFuture.runAsync(()
                                    -> generateUserLocationHistory(user))
                            .thenRun(() -> internalUserMap.put(user.getUserName(), user))
                    );
            futures.add(fullProcess);
        });
        return CompletableFuture
                .allOf(futures.toArray(new CompletableFuture[0]))
                .thenRun(() -> logger.debug("Created " + InternalTestHelper.getInternalUserNumber() + " internal test users."));
